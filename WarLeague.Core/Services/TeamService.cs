...
namespace WarLeague.Core.Services
{
    public class TeamService
    {
        ...
        public async Task<BaseResult> RenameAsync(int seasonId, string oldName, string newName, bool canBypassTeamModificationCheck)
        {
            var season = await _seasonRepository.GetSingleActiveSeasonByIdAsync(seasonId);
            ...
            if (team is null)
            {
                return new BaseResult { Success = false, Message = $"Team with name '{oldName}' does not exist in this season." };
            }

            team.Name = newName;
            await _teamRepository.UpdateAsync(team);

            return new BaseResult { Success = true, Message = $"Team renamed from '{oldName}' to '{newName}' successfully." };
        }
    }
}
