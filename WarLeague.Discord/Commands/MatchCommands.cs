...
public class MatchCommands : InteractionModuleBase<SocketInteractionContext>
{
        ...
        [Summary("replay-url", "URL to the replay")] string replayUrl)
    {
        ...
        {
            await RespondAsync("Invalid replay URL. Please provide a valid HTTP/HTTPS URL.", ephemeral: true);
            return;
        }
        ...
        {
            await RespondAsync($"User {opponent.Mention} is not registered as a player.", ephemeral: true);
            return;
        }
        ...
        if (match == null)
        {
            await RespondAsync("No match found between you and this opponent.", ephemeral: true);
            return;
        }
